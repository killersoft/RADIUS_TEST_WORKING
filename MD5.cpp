...
void testMD5() {
    ...
    unsigned char* md5Hash = md5.make_hash((const void *)data);
    ...
    // Example HMAC-MD5
    const char *key = "key";
    char* hmacHash = md5.hmac_md5(data, strlen(data), (void *)key, strlen(key));

    Serial.print("HMAC-MD5: ");
    Serial.println(hmacHash);  // Print the HMAC as a string

    free(md5Hash);  // Free the allocated memory if necessary
    free(hmacHash); // Free the allocated memory if necessary
}
*/
